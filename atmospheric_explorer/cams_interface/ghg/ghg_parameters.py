...
class GHGParameters(CAMSParameters):
    ...
    def years(self: GHGParameters) -> str | list[str]:
        ...
        return [str(y) for y in self._years] if len(self._years) > 1 else str(next(iter(self._years)))

    @years.setter
    def years(
        self: GHGParameters, years: str | set[str] | list[str]
    ) -> None:
    ...
    def months(self: GHGParameters) -> str | list[str]:
        ...
        return [f"{m:02}" for m in self._months] if len(self._months) > 1 else f"{next(iter(self._months)):02}"

    @months.setter
    def months(
        self: GHGParameters, months: str | set[str] | list[str]
    ) -> None:
        self._months = self._convert_to_intset(months)
    
    def __eq__(self, other: GHGParameters) -> bool:
        return (
            other.data_variables == self.data_variables and
            other.file_format == self.file_format and
            other.quantity == self.quantity and
            other.input_observations == self.input_observations and
            other.time_aggregation == self.time_aggregation and
            other.version == self.version and
            other._years == self._years and
            other._months == self._months
        )
    ...
    def difference(
        ...
        if (
            ...
            logger.debug("Parameters have the same key variables, moving to compute year and month difference")
