    ...
    def get_rows(cls, parameters: GHGParameters | None = None) -> list[dict]:
        with Session(cache_engine) as session:
            if parameters is not None:
                return [row._mapping[cls] for row in session.execute(
                    select(cls).where(
    ...
    def get_files(cls, parameters: list[GHGParameters]) -> list:
        ...
        with Session(cache_engine) as session:
            return session.scalars(
                select(cls.files_path).distinct().where(
                    ...
                    cls.input_observations.in_({p.input_observations for p in parameters}),
